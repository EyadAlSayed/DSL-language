...
public class ConditionVisitor extends DSLParserBaseVisitor {
    ...
    public Condition visitCondition(DSLParser.Main_conditionContext ctx) {
        ...
        if (ctx.equal_condition() != null && ctx.equal_condition().FILE_NAME_ID() != null)
        {
            ...
            condition.setFileNameId(ctx.equal_condition().FILE_NAME_ID().getText());
                ...
                try{
                    Files.writeString(ProjectMain.ERROR_FILE.toPath(), "SEMANTIC ERROR: VARIABLE " + ctx.equal_condition().FILE_NAME_ID().getText() + " IS NOT {TEXT,TEXTFIELD,CHECKBOX,RADIOBUTTON} OR DOES NOT EXIST!\n", StandardOpenOption.APPEND);
                } catch (IOException e){
                    e.printStackTrace();
                }
            }
